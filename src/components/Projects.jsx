...
function ProjectCard({ project, index }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -6 }}
      className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 flex flex-col gap-4 hover:border-violet-700/50 transition-colors group"
    >
      <div className="w-10 h-10 rounded-xl bg-violet-600/20 flex items-center justify-center text-violet-400 font-bold text-lg group-hover:bg-violet-600/30 transition-colors">
        {index + 1}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {project.tags.map((tag) => (
          <span key={tag} className="text-xs px-2 py-1 bg-neutral-800 text-neutral-400 rounded-md">
            {tag}
          </span>
        ))}
      </div>
...
export default function Projects() {
  return (
    <section id="projects" className="py-24 px-6 bg-neutral-900/30">
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-neutral-400">A few things I've built recently.</p>
        </motion.div>
